...
export function BusinessesPage({ tab = "active" }: { tab?: string }) {
  ...
  // Early Venture Studio (Discover/Validate) stays under Business and Brand Ideas until it is clearly building.
  const active = useMemo(() => businesses.filter(i => {
    if (ACTIVE.test(i.status) || ["Build", "Launch", "Scale"].includes(i.stage)) return true;
    if (i.ventureTrack === "Venture Studio" && !["Discover", "Validate"].includes(i.stage)) return true;
    return false;
  }), [businesses]);
  ...
  const copy = tab === "ideas"
    ? { icon: BriefcaseBusiness, title: "Business and Brand Ideas", text: "Promising business and brand models that still need exploration before you commit to building." }
    : tab === "products"
  ...
  return (
    <div className="page-theme page-theme-businesses">
      <PageHeader eyebrow="From Possibility to Company" title="Businesses"
        description="A clear home for business and brand ideas, active companies, and products—without separating them from the income horizons they also support."
